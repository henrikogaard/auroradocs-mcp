...
| Client shows no tools | The stdio process failed, is still connecting, or the client has stale configuration. | Inspect the client's MCP status, confirm the command is `npx -y auroradocs-mcp@0.1.0`, then restart the client. |
