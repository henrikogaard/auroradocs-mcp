...
const requiredReadmeText = [
  ...
  'E2EE',
  'stdio',
  '@henrikogard/auroradocs-mcp@0.3.0',
  'read:tasks',
  'write:tasks',
]
...
test('public docs use only the canonical scoped npm package', async () => {
  ...
  assert.match(docs, /npx -y @henrikogard\/auroradocs-mcp@0\.3\.0/)
...
test('public docs explain custom databases and the consent-gated local Obsidian boundary', async () => {
  ...
  assert.match(readme, /latest published package and current source version are\s+`0\.3\.0`/)
  assert.match(obsidian, /Available since 0\.2\.1/)
